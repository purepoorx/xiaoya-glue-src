...
for i in $SCAN_PATHS; do
	cp /strm3.py /tmp/strm3.py
	SCAN_PATH=$(head -n $i /strm.txt |tail -n 1|tr -d '\r'|cut -f2 -d= |sed 's/\//\\\//g; s/\s\+$//g; s/&/%26/g')
	sed -i "s/DOCKER_ADDRESS/$DOCKER_ADDRESS/" /tmp/strm3.py
	sed -i "s/SCAN_PATH/$SCAN_PATH/" /tmp/strm3.py
	sed -i "s/USERNAME/$USERNAME/" /tmp/strm3.py
	sed -i "s/PASSWORD/$PASSWORD/" /tmp/strm3.py
	/tmp/strm3.py
